...
fn solve_part1(input: &[InputLine]) -> usize {
    ...
    let mut confirmedIngredients: HashSet<String> = HashSet::new();
    ...
    while results.iter().any(|item| item.len() == 1) {
        let result = *results
            ...
            .first()
            .unwrap();
        confirmedIngredients.insert(result.clone());
        results = results
            .iter()
            .map(|alist| {
    ...
    let non_ingredients: HashSet<String> = all_ingredients
        .difference(&confirmedIngredients)
        .map(|item| item.clone())
        .collect();
...
fn solve_part2(input: &[InputLine]) -> String {
    ...
    let mut confirmedIngredients: Vec<(String, String)> = vec![];
    ...
    while results.iter().any(|item| item.1.len() == 1) {
        ...
        confirmedIngredients.push((result.0.clone(), ingredient.clone()));
        results = results
            .iter()
            .map(|alist| {
                let rr: Vec<String> = alist
    ...
    confirmedIngredients.sort_by(|a, b| a.0.cmp(&b.0));
    let string_list: Vec<String> = confirmedIngredients
        .iter()
