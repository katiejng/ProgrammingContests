...
const getIntersection = (array1, array2) => array1.filter(value => -1 !== array2.indexOf(value))
...
const converted = intersection.map(point => point.split(',').map(x=> parseInt(x)))
const distances = converted.map( point => Math.abs(point[0]) + Math.abs(point[1]))
